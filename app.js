...
function App() {
  ...
  const [darkModePref, setDarkModePref] = useState(() => {
    ...
    if (!match || match[1] === "auto") return "auto";
  ...
  const isDarkMode =
    darkModePref === "auto" ? systemDark : darkModePref === "dark";
  const setDarkModeCookie = (pref) => {
    setDarkModePref(pref);
    document.cookie = "darkMode=" + pref + "; path=/; max-age=31536000";
  };
  ...
  const [lagerFilterEnabled, setLagerFilterEnabled] = useState(() => {
    try {
      return localStorage.getItem("alchemyLagerFilter") === "1";
    } catch {
      return false;
    }
  });
  const toggleLagerFilter = () => {
    setLagerFilterEnabled((prev) => {
      const next = !prev;
      try {
        localStorage.setItem("alchemyLagerFilter", next ? "1" : "0");
      } catch {}
      return next;
    });
  };
  ...
  const toggleSimplifiedCraftView = () => {
    setSimplifiedCraftView((prev) => {
      const next = !prev;
      try {
        localStorage.setItem("alchemySimplifiedCraftView", next ? "1" : "0");
      } catch {}
      return next;
    });
  };
  ...
  const [focusMode, setFocusMode] = useState(() => {
    try {
      return localStorage.getItem("alchemyFocusMode") === "1";
    } catch {
      return false;
    }
  });
  const toggleFocusMode = () => {
    setFocusMode((prev) => {
      const next = !prev;
      try {
        localStorage.setItem("alchemyFocusMode", next ? "1" : "0");
      } catch {}
      return next;
    });
  };
  ...
  const [focusRootStep, setFocusRootStep] = useState(() => {
    try {
      return (
        parseInt(localStorage.getItem("alchemyFocusRootStep") || "0", 10) || 0
      );
    } catch {
      return 0;
    }
  ...
  const setFocusRootStepAndSave = (n) => {
    setFocusRootStep(n);
    try {
      localStorage.setItem("alchemyFocusRootStep", String(n));
    } catch {}
  };
  ...
  const getInitialSubRecipeMaterials = (recipeId) => {
    ...
    if (r) {
      const { combo } = findBestIngredientsWithFallback(
        r,
        filteredLager,
        disabledItems,
        ignoreEmptyItems,
  ...
  const handleExportSettings = () => {
    const STATIC_KEYS = [
      "alchemyCustomRecipes",
      "alchemyFavorites",
      "alchemyCart",
      "alchemyDisabledItems",
      "alchemyLowStockThreshold",
      "alchemySimplifiedDelivery",
      "alchemySimplifiedCraftView",
      "alchemyStep4RecipeIdx",
      "alchemyIgnoreEmptyItems",
      "alchemyRememberIngredients",
      "alchemyFocusMode",
      "alchemyFocusRootStep",
      "alchemySplitMode",
      "alchemySession",
      "alchemyLager",
      "alchemyLagerData",
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (k && k.startsWith("alchemyIngredients_"))
    ...
    a.href = url;
    a.download = "alchemia-settings.json";
    a.click();
  ...
  const [foundSecrets, setFoundSecrets] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("alchemyFoundSecrets") || "[]");
    } catch {
      return [];
    }
  });
  const [secretFoundToast, setSecretFoundToast] = useState(null);
  const secretFoundToastTimer = React.useRef(null);
  ...
  const [sessionMaxStep, setSessionMaxStep] = useState(
    () => _savedSession?.maxStep ?? _savedSession?.step ?? 1,
  );
  ...
  const [sessionSwapQualityFilter, setSessionSwapQualityFilter] =
    useState(null);
  const [sessionSwapSearchQuery, setSessionSwapSearchQuery] = useState("");
  ...
  const [sessionCatalysts, setSessionCatalysts] = useState(() => {
    if (_savedSession?.catalystNames)
      return _savedSession.catalystNames.map((n) => n ?? null);
    return [];
  });
  ...
  const removeSubRecipe = (idx) => {
    ...
    const collect = (id) => {
      ...
      sessionRecipes
        ...
        .forEach((r) => collect(r._id));
    };
    collect(entry._id);
    const activeId = sessionRecipes[sessionActiveIdx]?._id;
    if (toRemove.has(activeId)) {
      const parentIdx = sessionRecipes.findIndex(
        (r) => r._id === entry.parentId,
      );
      ...
      if (sm && sm.some((id) => id != null)) {
        ...
        const parentRecipe = [...RECIPES, ...customRecipes].find(
          (r) => r.id === parentEntry?.recipeId,
        );
        if (parentRecipe) {
          const { combo } = findBestIngredientsWithFallback(
            parentRecipe,
    ...
    const removeIndices = new Set(
      sessionRecipes.reduce((a, r, i) => {
        if (toRemove.has(r._id)) a.push(i);
        return a;
      }, []),
    );
    ...
    try {
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          catalystNames: sessionCatalysts.map((n) => n ?? null),
  ...
  const sessionAllMaterials = React.useMemo(() => {
    ...
    sessionRecipes.forEach(({ recipeId, savedMaterials }) => {
      ...
      if (savedMaterials && savedMaterials.some((id) => id != null)) {
        ...
        materials =
          findBestIngredients(recipe, 0, filteredLager, disabledItems) || [];
      }
    ...
    sessionAgents.forEach((agent, idx) => {
      if (agent && sessionRecipes[idx])
        matNeeded[agent.id] = (matNeeded[agent.id] || 0) + 1;
    });
    return matNeeded;
  }, [
    sessionRecipes,
  ...
  const sessionSelectRecipeSlot = (idx) => {
    // Focus-Mode: focusRootStep auf die DFS-Position des angeklickten Rezepts setzen
    if (focusMode) {
      ...
      const rts = sessionRecipes.reduce((acc, r, i) => {
        if (!r.parentId || !kIds.has(r.parentId)) acc.push(i);
        return acc;
      }, []);
      const ord = [];
      const vis = (i) => {
        ord.push(i);
        (co[sessionRecipes[i]._id] || []).forEach(vis);
      };
      rts.forEach(vis);
      const pos = ord.indexOf(idx);
      if (pos >= 0) setFocusRootStepAndSave(pos);
    }
    ...
    // Catalyst für altes Rezept speichern
    setSessionCatalysts((prev) => {
      const next = [...prev];
      while (next.length <= sessionActiveIdx) next.push(null);
      next[sessionActiveIdx] = catalyst.name;
      return next;
    });
    ...
    // Catalyst für neues Rezept wiederherstellen:
    // 1. gespeicherter Catalyst aus sessionCatalysts
    // 2. sonst: catalyst-Feld aus Custom-Rezept
    const newRecipeForCat = [...RECIPES, ...customRecipes].find(
      (r) => r.id === entry?.recipeId,
    );
    const savedCatName =
      sessionCatalysts[idx] ??
      (newRecipeForCat?.rank === "Custom" ? newRecipeForCat.catalyst : null);
    if (savedCatName) {
      const cat = CATALYSTS.find((c) => c.name === savedCatName);
      if (cat) setCatalyst(cat);
    }
  };
  const sessionGoToStep3 = () => {
    ...
    // Catalyst des aktiven Rezepts sichern
    setSessionCatalysts((prev) => {
      const next = [...prev];
      while (next.length <= sessionActiveIdx) next.push(null);
      next[sessionActiveIdx] = catalyst.name;
  ...
  const cartPerRecipe = useMemo(() => {
    return cart.map(({ recipeId, quantity, savedMaterials, savedAgentId }) => {
      ...
      const catObj = CATALYSTS.find((c) => c.name === recipe.catalyst);
      if (catObj && catObj.id !== "none")
        matNeeded[catObj.id] = (matNeeded[catObj.id] || 0) + quantity;
      if (savedAgentId != null) {
        const agentObj = AGENTS.find((a) => a.id === savedAgentId);
        if (agentObj)
          matNeeded[agentObj.id] = (matNeeded[agentObj.id] || 0) + quantity;
      }
      return matNeeded;
    });
  }, [cart, customRecipes, filteredLager, disabledItems]);
  const cartTotal = useMemo(() => {
    const matNeeded = {};
    cart.forEach(({ recipeId, quantity, savedMaterials, savedAgentId }) => {
      if (quantity <= 0) return;
      const recipe = [...RECIPES, ...customRecipes].find(
        (r) => r.id === recipeId,
      );
  ...
  const getLagerAmount = (id) => filteredLager[id] || 0;
  ...
  // Wenn der Agent sich ändert und das aktive Rezept im Korb ist, savedAgentId aktualisieren
  useEffect(() => {
    if (!selectedRecipe) return;
    setCart((prev) => {
      const idx = prev.findIndex((i) => i.recipeId === selectedRecipe.id);
  ...
  const handleSelectRecipe = (recipe) => {
    ...
    if (
      ...
      setCatalyst(neededCatalyst);
      const restoredAgent =
        cartEntry.savedAgentId != null
          ? AGENTS.find((a) => a.id === cartEntry.savedAgentId) || null
          : null;
      setSelectedAgent(restoredAgent);
      setSelectedRecipe(recipe);
      return;
  ...
  const lagerWarnings = useMemo(() => {
    const missing = [];
    const low = [];
    if (Object.keys(filteredLager).length === 0)
      return {
        missing,
        low,
      };
    cauldron.forEach((slot, slotIdx) => {
      if (!slot) return;
      const amt = filteredLager[slot.id] || 0;
      if (amt === 0) {
        ...
        const alternative =
          MATERIALS.filter(
            (m) =>
              m.type === slot.type &&
              m.id !== slot.id &&
              (filteredLager[m.id] || 0) > 0,
          ).sort(
            (a, b) => (filteredLager[b.id] || 0) - (filteredLager[a.id] || 0),
          )[0] || null;
        missing.push({
          material: slot,
          slotIdx,
  ...
  }, [cauldron, filteredLager, lowStockThreshold]);
  ...
  const tutMockLagerPreview =
    ...
    tutorialState?.steps?.[tutorialState?.step]?.target !==
      "tut-lager-edit-btn" &&
    Object.keys(filteredLager).length === 0
      ? (() => {
          const mats = window.MATERIALS || [];
        ...
        /*#__PURE__*/ React.createElement(
          ...
          /*#__PURE__*/ React.createElement(
            ...
            /*#__PURE__*/ React.createElement(
              ...
              {
                className:
                  "px-4 pt-3 pb-2 bg-slate-50 dark:bg-slate-900 border-b border-slate-100 dark:border-slate-700 flex-shrink-0",
              },
              /*#__PURE__*/ React.createElement(
                "div",
                { className: "relative" },
                /*#__PURE__*/ React.createElement("input", {
                  ...
                  className:
                    "w-full px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm text-slate-700 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:focus:ring-indigo-500 pr-8",
                }),
                swapSearchQuery &&
                  /*#__PURE__*/ React.createElement(
                    "button",
                ...
                MATERIALS.filter(
                  ...
                  .filter(
                    (m) =>
                      ...
                      m.name
                        .toLowerCase()
                        .includes(swapSearchQuery.toLowerCase()),
                  )
                  ...
                  .map((material) => {
                    ...
                    const lagerAmt = filteredLager[material.id] || 0;
                    let dynamicBg =
      ...
      /*#__PURE__*/ React.createElement(
            ...
            /*#__PURE__*/ React.createElement(
              ...
              ["auto", "dark", "light"].map((pref) =>
                /*#__PURE__*/ React.createElement(
                  ...
                  {
                    key: pref,
                    onClick: () => setDarkModeCookie(pref),
                  ...
                  /*#__PURE__*/ React.createElement("i", {
                    className:
                      pref === "auto"
                        ? "fa-solid fa-circle-half-stroke text-sm"
                        : pref === "dark"
        ...
        stockAlertOpen &&
          (() => {
            ...
            // 3. Vergleich mit filteredLager
            const knapp = [];
            const leer = [];
            pool.forEach((item) => {
              const amount = filteredLager[item.id];
        ...
        secretFoundToast !== null &&
          /*#__PURE__*/ React.createElement(
            "div",
            {
              className:
                "fixed bottom-6 right-6 z-[61] flex flex-col gap-1 px-4 py-3 rounded-xl shadow-lg bg-yellow-100 dark:bg-yellow-900/80 border border-yellow-300 dark:border-yellow-700 text-yellow-800 dark:text-yellow-300 pointer-events-none",
            },
            /*#__PURE__*/ React.createElement(
              "div",
            ...
            /*#__PURE__*/ React.createElement(
              ...
              /*#__PURE__*/ React.createElement(
                  ...
                  /*#__PURE__*/ React.createElement(
                    ...
                    /*#__PURE__*/ React.createElement(
                      ...
                      /*#__PURE__*/ React.createElement(
                        "p",
                        {
                          className: `text-sm font-bold mb-0.5 ${!characterName.trim() ? "text-slate-400 dark:text-slate-500" : "text-slate-700 dark:text-slate-200"}`,
                        },
                        t("lagerFilterLabel"),
                      ),
                      /*#__PURE__*/ React.createElement(
                    ...
                    /*#__PURE__*/ React.createElement(
                      "button",
                      {
                        onClick: characterName.trim()
                          ? toggleLagerFilter
                          : undefined,
                        disabled: !characterName.trim(),
                        className: `relative inline-flex h-6 w-11 flex-shrink-0 rounded-full border-2 border-transparent transition-colors focus:outline-none ${!characterName.trim() ? "opacity-40 cursor-not-allowed bg-slate-300 dark:bg-slate-600" : lagerFilterEnabled ? "bg-indigo-500" : "bg-slate-300 dark:bg-slate-600"}`,
                      },
                      /*#__PURE__*/ React.createElement("span", {
                        className: `inline-block h-5 w-5 transform rounded-full bg-white shadow transition-transform ${lagerFilterEnabled && characterName.trim() ? "translate-x-5" : "translate-x-0"}`,
                      }),
                    ),
                  ),
                ),
                ...
                /*#__PURE__*/ React.createElement(
                  ...
                  {
                    className:
                      "p-4 mb-6 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700",
                  },
                  /*#__PURE__*/ React.createElement(
                    "p",
                    {
                  ...
                  /*#__PURE__*/ React.createElement(
                    ...
                    /*#__PURE__*/ React.createElement(
                      "button",
                      {
                        ...
                        className:
                          "flex items-center gap-2 px-4 py-2 text-sm font-bold bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg transition-colors",
                      },
                      /*#__PURE__*/ React.createElement("i", {
                    ...
                    /*#__PURE__*/ React.createElement(
                      "button",
                      {
                        ...
                        className:
                          "flex items-center gap-2 px-4 py-2 text-sm font-bold bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 rounded-lg transition-colors",
                      },
                      /*#__PURE__*/ React.createElement("i", {
                    ...
                    [1, 2, 3, 4, 5, 6].map((s, i) =>
                      /*#__PURE__*/ React.createElement(
                        ...
                        /*#__PURE__*/ React.createElement(
                          ...
                          {
                            onClick: () => {
                              if (s > sessionMaxStep) return;
                              if (s === 2) sessionSelectRecipeSlot(0);
                              setSessionStep(s);
                            },
                  ...
                  /*#__PURE__*/ React.createElement(
                      ...
                      /*#__PURE__*/ React.createElement(
                        "div",
                        {
                          ...
                          style: focusMode
                            ? {
                                display: "grid",
                                gridTemplateColumns: "25% 5% 40% 5% 25%",
                                alignItems: "start",
                              }
                            : undefined,
                        },
                        (() => {
                          ...
                          const renderTabBtn = (idx, depth) => {
                            ...
                            const catObjS2 =
                              recipe?.catalyst && recipe.catalyst !== "none"
                                ? CATALYSTS.find(
                                    (c) => c.name === recipe.catalyst,
                                  )
                                : null;
                            if (catObjS2?.id && catObjS2.id !== "none")
                              slotsNeeded[catObjS2.id] =
                                (slotsNeeded[catObjS2.id] || 0) + 1;
                            if (tabAgent?.id)
                              slotsNeeded[tabAgent.id] =
                                (slotsNeeded[tabAgent.id] || 0) + 1;
                            ...
                            (childrenOf[entry._id] || []).forEach(
                              (childIdx) => {
                                ...
                                const childRecipe = [
                                  ...RECIPES,
                                  ...customRecipes,
                                ].find((r) => r.id === childEntry.recipeId);
                                if (childRecipe?.product) {
                                  const found =
                                    MATERIALS.find(
                                      (m) => m.name === childRecipe.product,
                                    ) ||
                                    AGENTS.find(
                                      (a) => a.name === childRecipe.product,
                                    ) ||
                                    CATALYSTS.find(
                                      (c) =>
                                        c.id !== "none" &&
                                        c.name === childRecipe.product,
                                    );
                                  if (found)
                                    subCraftProducts[found.id] =
                                      (subCraftProducts[found.id] || 0) + 1;
                                }
                              },
                            );
                            const lagerOk =
                              ...
                              Object.entries(slotsNeeded).every(
                                ([id, needed]) =>
                                  (filteredLager[parseInt(id)] || 0) +
                                    (subCraftProducts[parseInt(id)] || 0) >=
                                  needed,
                              );
                            const recipeValid =
                              recipe &&
                            ...
                            const isRoot =
                              !entry.parentId || !knownIds.has(entry.parentId);
                            const sizeClass =
                              depth === 0
                                ? "text-sm py-1.5 px-3"
                                : "text-xs py-1 px-2.5";
                            return /*#__PURE__*/ React.createElement(
                              "button",
                              {
                                ...
                                onDrop: isRoot
                                  ? (e) => {
                                      e.preventDefault();
                                      if (sessionDragIdx !== null)
                                        sessionReorderRecipes(
                                          sessionDragIdx,
                                          idx,
                                        );
                              ...
                              !isRoot &&
                                /*#__PURE__*/ React.createElement(
                                  "span",
                                  {
                                    ...
                                    className:
                                      "ml-0.5 opacity-60 hover:opacity-100 hover:text-red-400 transition-opacity leading-none",
                                  },
                                  "\u00d7",
                                ),
                            );
                          ...
                          const currentRootPosInRoots =
                            roots.indexOf(currentRootIdx);
                          const prevRootIdx =
                            currentRootPosInRoots > 0
                              ? roots[currentRootPosInRoots - 1]
                              : null;
                          const nextRootIdx =
                            currentRootPosInRoots < roots.length - 1
                              ? roots[currentRootPosInRoots + 1]
                              : null;
                          const ghostBtn = (ri, isPrev) => {
                            ...
                            return /*#__PURE__*/ React.createElement(
                              "button",
                              {
                                ...
                                className:
                                  "flex items-center gap-2 text-sm py-1.5 px-3 rounded-xl border font-bold transition-all opacity-40 hover:opacity-70 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 cursor-pointer flex-shrink-0",
                              },
                              /*#__PURE__*/ React.createElement(ItemIcon, {
                          ...
                          const makeArrow = () =>
                            /*#__PURE__*/ React.createElement("i", {
                              className:
                                "fa-solid fa-chevron-right text-slate-500 dark:text-slate-400 text-xs",
                              style: { lineHeight: "1" },
                            });
                          return /*#__PURE__*/ React.createElement(
                            React.Fragment,
                            ...
                            /*#__PURE__*/ React.createElement(
                              ...
                              /*#__PURE__*/ React.createElement(
                                "button",
                                {
                                  onClick: () => {
                                    const { combo, agent } =
                                      findBestIngredientsWithFallback(
                                        recipe,
                                        filteredLager,
                                        disabledItems,
                                        ignoreEmptyItems,
                                    ...
                                    if (agent != null) {
                                      setSessionAgents((prev) => {
                                        const next = [...prev];
                                        while (next.length <= sessionActiveIdx)
                                          next.push(null);
                                        next[sessionActiveIdx] = agent;
                                        return next;
                                      });
                                  ...
                                  title: t("autoSelectBtn"),
                                  className:
                                    "flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-bold text-indigo-500 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors",
                                },
                                /*#__PURE__*/ React.createElement("i", {
                                  className:
                                    "fa-solid fa-wand-magic-sparkles text-xs",
                                    ...
                                    slot
                                      ? /*#__PURE__*/ React.createElement(
                                          ...
                                          /*#__PURE__*/ React.createElement(
                                            ...
                                            Object.keys(filteredLager).length >
                                              0 &&
                                              /*#__PURE__*/ React.createElement(
                                                "p",
                                                {
                                        ...
                                        onClick: (e) => {
                                          ...
                                          if (craftableRecipes.length === 1) {
                                            setSessionRecipes((prev) => [
                                              ...
                                              {
                                                recipeId:
                                                  craftableRecipes[0].id,
                                                savedMaterials:
                                                  getInitialSubRecipeMaterials(
                                                    craftableRecipes[0].id,
                                                  ),
                                                _id: Math.random()
                                                  .toString(36)
                                ...
                                /*#__PURE__*/ React.createElement(
                                  ...
                                  (() => {
                                    ...
                                    const _al =
                                      sessionAgent &&
                                      Object.keys(filteredLager).length > 0
                                        ? (filteredLager[sessionAgent.id] ?? 0)
                                        : null;
                                            ...
                                            ? /*#__PURE__*/ React.createElement(
                                                ...
                                                /*#__PURE__*/ React.createElement(
                                                  ...
                                                  /*#__PURE__*/ React.createElement(
                                                    ...
                                                    Object.keys(filteredLager)
                                                      .length > 0 &&
                                                      /*#__PURE__*/ React.createElement(
                                                        "div",
                                                        {
                                                          className:
                                                            "flex items-center gap-0.5 py-0.5 px-1 rounded bg-white/70 dark:bg-slate-700/60 text-[10px] font-bold text-slate-500 dark:text-slate-400",
                                                        },
                                                        /*#__PURE__*/ React.createElement(
                                                          "i",
                                                          {
                                                            className:
                                                              "fa-solid fa-box text-[9px]",
                                                          },
                                                        ),
                                                        filteredLager[
                                                          sessionAgent.id
                    ...
                    /*#__PURE__*/ React.createElement(
                        ...
                        "button",
                        {
                          onClick: () => {
                            if (focusMode && focusRootStep > 0) {
                              ...
                              sessionRecipes.forEach((r, i) => {
                                if (r.parentId) {
                                  if (!coB[r.parentId]) coB[r.parentId] = [];
                                  coB[r.parentId].push(i);
                                }
                              });
                              const rB = sessionRecipes.reduce((a, r, i) => {
                                if (!r.parentId || !kB.has(r.parentId))
                                  a.push(i);
                                return a;
                              }, []);
                              const dB = [];
                              const vB = (idx) => {
                                dB.push(idx);
                                (coB[sessionRecipes[idx]._id] || []).forEach(
                                  vB,
                                );
                              };
                              rB.forEach(vB);
                              ...
                              if (dB[prevStep] !== undefined)
                                sessionSelectRecipeSlot(dB[prevStep]);
                            } else {
                              setSessionStep(1);
                            }
                          },
                          onMouseDown:
                            focusMode && focusRootStep > 0
                              ? (e) => {
                                  e.preventDefault();
                                  startHold("s2-back", () => setSessionStep(1));
                                }
                              : undefined,
                          onMouseUp:
                            focusMode && focusRootStep > 0
                      ...
                      (() => {
                        ...
                        const isLastStep =
                          !focusMode ||
                          focusRootStep >= focusDfsOrder.length - 1;
                        ...
                        return /*#__PURE__*/ React.createElement(
                          "button",
                          {
                            ...
                            disabled: !btnEnabled,
                            title: !btnEnabled
                              ? "Nicht alle Rezepte erfüllen die Anforderungen"
                              : "",
                            onMouseDown:
                              btnEnabled && focusEnabled
                                ? (e) => {
                                    e.preventDefault();
                                    startHold("s2-next", () =>
                                      sessionGoToStep3(),
                          ...
                          /*#__PURE__*/ React.createElement(
                            ...
                            focusEnabled &&
                              /*#__PURE__*/ React.createElement(
                                ...
                                {
                                  className:
                                    "text-indigo-200 text-xs font-bold tabular-nums",
                                },
                                `${focusRootStep + 1}/${focusDfsOrder.length}`,
                              ),
                            focusEnabled ? t("focusModeNext") : t("next"),
                            " ",
                            /*#__PURE__*/ React.createElement("i", {
                              className: "fa-solid fa-arrow-right",
                            }),
                          ...
                          "button",
                          {
                            ...
                            onClick: () => {
                              setSessionRecipes((prev) => [
                                ...
                                {
                                  recipeId: r.id,
                                  savedMaterials: getInitialSubRecipeMaterials(
                                    r.id,
                                  ),
                                  _id: Math.random().toString(36).slice(2, 10),
                        ...
                        Object.keys(sessionAllMaterials).length === 0
                          ...
                          : (() => {
                              ...
                              const renderEntry = ([idStr, needed]) => {
                                ...
                                const inLager = filteredLager[id] || 0;
                              ...
                              const craftingEntries = allEntries.filter(
                                ([idStr]) =>
                                  craftingProductIds.has(parseInt(idStr)),
                              );
                              const nonCraftingEntries = allEntries.filter(
                                ([idStr]) =>
                                  !craftingProductIds.has(parseInt(idStr)),
                              );
                              ...
                              if (matSplitMode === "stock") {
                                const available = nonCraftingEntries.filter(
                                  ([idStr, needed]) =>
                                    (filteredLager[parseInt(idStr)] || 0) >=
                                    needed,
                                );
                        ...
                        (() => {
                          ...
                          sessionRecipes.forEach((entry, idx) => {
                            ...
                            if (!saved || saved.every((id) => id == null))
                              return;
                            const slotTypes = saved.map((id) =>
                              id != null
                                ? (MATERIALS.find((m) => m.id === id)?.type ??
                                  null)
                                : null,
                            );
                            const allFilled = saved.every((id) => id != null);
                            const agent = sessionAgents[idx] ?? null;
                            window.SECRET_RECIPES.forEach((s) => {
                              if (!foundSecrets.includes(s.id)) return;
                              if (
                                s.mainIngredient !== undefined &&
                                saved[0] !== s.mainIngredient
                              )
                                return;
                              if (allFilled) {
                                if (
                                  !s.ingredients.every(
                                    (type, i) => slotTypes[i] === type,
                                  )
                                )
                                  return;
                              }
                          ...
                          return /*#__PURE__*/ React.createElement(
                            ...
                            /*#__PURE__*/ React.createElement(
                              ...
                              {
                                className:
                                  "text-xs font-bold uppercase tracking-wider text-yellow-600 dark:text-yellow-400 mb-2 flex items-center gap-1",
                              },
                              /*#__PURE__*/ React.createElement("i", {
                              ...
                              allExtraItems.map((item) =>
                                /*#__PURE__*/ React.createElement(
                                  "div",
                                  {
                                    key: item.id,
                                    className:
                                      "flex items-center gap-2 px-3 py-2 rounded-xl border bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-700",
                                  },
                                  TYPE_ICONS[item.type]
                                    ...
                                    : /*#__PURE__*/ React.createElement("i", {
                                        className:
                                          "fa-solid fa-star w-6 h-6 text-yellow-500 flex-shrink-0",
                                      }),
                                  /*#__PURE__*/ React.createElement(
                                    "span",
                                    {
                      ...
                      ? (() => {
                          ...
                          const recipeOk =
                            ...
                            Object.entries(perMats).every(
                              ([idStr, needed]) =>
                                (filteredLager[parseInt(idStr)] || 0) >= needed,
                            );
                          const renderSlots = (ent, rec, ri) => {
                            const pm = sessionPerRecipeMaterials[ri] || {};
                            const effectiveRank =
                              rec.rank === "Custom" ? rec.customRank : rec.rank;
                            const potionId = {
                              Basic: 645,
                              Intermediate: 656,
                              Advanced: 657,
                              ...
                              (matId, slotIdx) => {
                                ...
                                slots.push({
                                  ...
                                  name:
                                    mat?.name ||
                                    (matId != null ? `ID ${matId}` : "—"),
                                  needed: matId != null ? pm[matId] || 1 : 0,
                                });
                              },
                            );
                            if (catObj && catObj.id !== "none")
                            ...
                            /*#__PURE__*/ React.createElement(
                              ...
                              /*#__PURE__*/ React.createElement(
                                "button",
                                {
                                  onClick: () =>
                                    safeIdx > 0
                                      ? setSessionStep4RecipeIdxPersist(
                                          safeIdx - 1,
                                        )
                                      : setSessionStep(3),
                                  onMouseDown:
                                    safeIdx > 0
                                      ...
                                      : undefined,
                                  onMouseUp:
                                    safeIdx > 0 ? cancelHold : undefined,
                                  onMouseLeave:
                                    safeIdx > 0 ? cancelHold : undefined,
                                  onTouchStart:
                                    safeIdx > 0
              ...
              return /*#__PURE__*/ React.createElement(
                ...
                /*#__PURE__*/ React.createElement(
                  ...
                  /*#__PURE__*/ React.createElement(
                    ...
                    {
                      className:
                        "px-4 pt-3 pb-2 bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-700 flex-shrink-0",
                    },
                    /*#__PURE__*/ React.createElement(
                      "div",
                      { className: "relative" },
                      /*#__PURE__*/ React.createElement("input", {
                        type: "text",
                        autoFocus: true,
                        value: sessionSwapSearchQuery,
                        onChange: (e) =>
                          setSessionSwapSearchQuery(e.target.value),
                        placeholder: t("ingredientSearch"),
                        className:
                          "w-full px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm text-slate-700 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:focus:ring-indigo-500 pr-8",
                      }),
                      sessionSwapSearchQuery &&
                        /*#__PURE__*/ React.createElement(
                          "button",
                          {
                            onClick: () => setSessionSwapSearchQuery(""),
                            className:
                              "absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200",
                          },
                          /*#__PURE__*/ React.createElement("i", {
                      ...
                      MATERIALS.filter(
                        ...
                        .filter(
                          (m) =>
                            ...
                            m.name
                              .toLowerCase()
                              .includes(sessionSwapSearchQuery.toLowerCase()),
                        )
                        .sort((a, b) => a.quality - b.quality)
                        .map((material) => {
                          const maxStat = Math.max(
            ...
            /*#__PURE__*/ React.createElement(
              ...
              lagerOpen
                ...
                : /*#__PURE__*/ React.createElement(
                    ...
                    Object.keys(tutMockLagerPreview || filteredLager).length ===
                      0
                      ? /*#__PURE__*/ React.createElement(
                          "div",
                          {
                      ...
                      : Object.entries(tutMockLagerPreview || filteredLager)
                          .sort(([a], [b]) => {
                        ...
                        /*#__PURE__*/ React.createElement(
                          ...
                          cart.map(
                            (
                              ...
                              const isExpanded =
                                cartExpandedRecipe === recipeId;
                              const perMats = cartPerRecipe[cartIdx] || {};
                              const _matsOrdered = (() => {
                                if (
                                  savedMaterials &&
                                  savedMaterials.some((id) => id != null)
                                )
                                  return savedMaterials.map((id) =>
                                    id != null
                                      ? MATERIALS.find((m) => m.id === id)
                                      : null,
                                  );
                                if (
                                  recipe.rank === "Custom" &&
                                  recipe.savedMaterials
                                )
                                  return recipe.savedMaterials.map((id) =>
                                    id != null
                                      ? MATERIALS.find((m) => m.id === id)
                                      : null,
                                  );
                              ...
                              const _sortOrder = {};
                              [645, 656, 657, 610].forEach(
                                (id) => (_sortOrder[id] = 0),
                              );
                              _matsOrdered.forEach((mat, i) => {
                                if (mat) _sortOrder[mat.id] = i + 1;
                              });
                              const _catSort = CATALYSTS.find(
                                (c) => c.name === recipe.catalyst,
                              );
                              if (_catSort && _catSort.id !== "none")
                                _sortOrder[_catSort.id] = 5;
                              if (savedAgentId != null) {
                                const _agentSort = AGENTS.find(
                                  (a) => a.id === savedAgentId,
                                );
                                if (_agentSort) _sortOrder[_agentSort.id] = 6;
                              }
                              return /*#__PURE__*/ React.createElement(
                                "div",
                                {
                                    ...
                                    (() => {
                                      ...
                                      if (_agentRow)
                                        _rows.push({
                                          id: _agentRow.id,
                                          qty: quantity,
                                          key: `agent-${_agentRow.id}`,
                                        });
                                      return _rows.map(({ id, qty, key }) => {
                                        const mat = findItem(id);
                                        const inLager = filteredLager[id] || 0;
                                        const totalNeeded = perMats[id] || qty;
                                        const ok =
                                          noLager || inLager >= totalNeeded;
                                        return /*#__PURE__*/ React.createElement(
                                          "div",
                                          {
                                            key,
                                          ...
                                          [
                                            ...
                                            /*#__PURE__*/ React.createElement(
                                              ...
                                              {
                                                className:
                                                  "text-[10px] px-1 leading-4 rounded border border-current opacity-60 font-normal flex-shrink-0",
                                              },
                                              mat.type,
                                            ),
                                          /*#__PURE__*/ React.createElement(
                  ...
                  matSplitMode !== "recipe"
                    ? (() => {
                        ...
                        const renderEntry = ([idStr, needed]) => {
                          ...
                          const mat = findItem(id);
                          const inLager =
                            (tutMockLager || filteredLager)[id] || 0;
                    ...
                    : cart.map(
                        (
                          { recipeId, quantity, savedMaterials, savedAgentId },
                          cartIdx,
                        ) => {
                          ...
                          _matsOrdered2.forEach((mat, i) => {
                            if (mat) _sortOrder2[mat.id] = i + 1;
                          });
                          const _catSort2 = CATALYSTS.find(
                            (c) => c.name === recipe.catalyst,
                          );
                          if (_catSort2 && _catSort2.id !== "none")
                            _sortOrder2[_catSort2.id] = 5;
                          if (savedAgentId != null) {
                            const _agentSort2 = AGENTS.find(
                              (a) => a.id === savedAgentId,
                            );
                            if (_agentSort2) _sortOrder2[_agentSort2.id] = 6;
                          }
                          const noLager =
                            Object.keys(filteredLager).length === 0;
                          const groupOk =
                          ...
                          return /*#__PURE__*/ React.createElement(
                            ...
                            /*#__PURE__*/ React.createElement(
                              ...
                              (() => {
                                ...
                                const _catRow2 = CATALYSTS.find(
                                  (c) => c.name === recipe.catalyst,
                                );
                                const _agentRow2 =
                                  savedAgentId != null
                                    ? AGENTS.find((a) => a.id === savedAgentId)
                                    : null;
                                const _rows2 = [];
                                if (_potionId2)
                                  _rows2.push({
                                ...
                                if (_agentRow2)
                                  _rows2.push({
                                    id: _agentRow2.id,
                                    qty: quantity,
                                    key: `agent-${_agentRow2.id}`,
                                  });
                                return _rows2.map(({ id, qty, key }) => {
                                  const mat = findItem(id);
                                  const inLager = filteredLager[id] || 0;
                                  const totalNeeded = perMats[id] || qty;
                                  const ok = noLager || inLager >= totalNeeded;
                                  return /*#__PURE__*/ React.createElement(
                                    "div",
                                    {
                                      key,
                      ...
                      return /*#__PURE__*/ React.createElement(
                        ...
                        slot &&
                          ...
                          /*#__PURE__*/ React.createElement(
                            ...
                            (tutMockLagerPreview || filteredLager)[slot.id] ??
                              0,
                          ),
                        slot
                          ? /*#__PURE__*/ React.createElement(
